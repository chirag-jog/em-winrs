  ...
  module WinRS
    class Session
      ...
      def initialize(options={})
        if options[:logger]
          WinRS::Log.logger = options[:logger]
        else
          log_level = options[:log_level] || :info
          WinRS::Log.level(log_level)
        end
        @servers = {}
        @commands = []
        @exit_codes = {}
        WinRS::Log.debug(":session => :init")
      end
      ...
      def relay_command(data)
        EM.epoll
        EM.run do
          WinRS::Log.debug(":relay_to_servers => #{data}")
          servers ||= @servers.values.compact
          servers.each do |s|
            operation = proc do
              WinRS::Log.debug(":relayed => #{s.host}")
      ...
      #
      # returns a new EventMachine::WinRS::Session instance
      ...
      def relay_output_from_backend(host, data)
        WinRS::Log.debug(":relay_output_from_backend => #{[host, data]}")
      ...
      def relay_error_from_backend(host, data)
        WinRS::Log.debug(":relay_error_from_backend => #{[host, data]}")
      ...
      def command_complete(host, cid,exit_code)
        WinRS::Log.debug(":command_complete => #{host} with return code #{exit_code}")
        @commands.delete(cid)
      ...
      def unbind
        WinRS::Log.debug(":unbind => :connection")
      ...
      def unbind_backend(host)
        WinRS::Log.debug(":unbind_backend => #{host}")
        @servers[host] = nil
