...
An EventMachine client for Windows Remote Management Shell (WinRS). This allows you to
invoke commands on Windows machines from any Windows machine that can run Ruby.  This
...
WinRS allows you to call native objects in Windows.  This includes, but is not
limited to, running batch scripts, powershell scripts and fetching WMI variables.
For more information on WinRS, please visit
{Microsoft's WinRS site}[http://msdn.microsoft.com/en-us/library/aa384426(v=VS.85).aspx].
...
= Example Usage
    ...
    # create a session
    session = EventMachine::WinRS::Session.new(:log_level => :info)
...
= Credit
This library is a clone of {Seth Chisamore}[http://github.com/schisamo]'s em-wimrm
